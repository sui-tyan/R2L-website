import image from '@/images/resources/events-wp3-outreach.jpg';

const text1 =
  'The community outreach in Barangay Loacan, Sitio Tocmo in Itogon, Benguet went full gear as teachers of the Tocmo Integrated School together with local government officials, and University of the Cordilleras officials did a Strategic Planning on April 26, 2023. The University of the Cordilleras delegation was led by Vice President and Rights2LIFE project leader, Dr. Ariel Nimo Pumecha and the College of Criminal Justice Education Dean, Dr. Robino Cawi. The planning activity facilitated by Dr. Darlyn Tagarino, resulted in a clear identification of the targets that the community partnership will achieve in the next three years.';
const text2 =
  'Principal Rinan Bruno and UC contingents led by Dr. Ariel Nimo B. Pumecha having a discussion on community activities and updates. Dr. Leah M. Donato engaging with high school students of Tocmo Integrated School on --. Criminology interns having a collaborative discussion with elementary pupils of Tocmo Integrated School on Anti-bullying and discipline.';
const text3 =
  'Mr. Scyld C. Segundo discussing the partnership and the Rights2LIFE project to the parents. Ms. Nica Togana also discussed parental rights and responsibilities in relation to rights of children. Left photo shows the ceremonial turn-over of books to the Principal and the PTA President. Right photo shows the awarding of certificate of appreciation to UC and Rights2LIFE .';

const eventsDetailsWP3 = [
  {
    id: 1,
    image,
    date: '4/26/23',
    title: 'Loakan, Tocmo, Itogon Community Outreach',
    texts: [text1],
  },
  {
    id: 2,
    image: image,
    date: '12/04/23',
    title:
      'TOCMO INTEGRATED SCHOOL Information, Education and Communication Drive and Sports Clinic',
    texts: [text2],
  },
  {
    id: 3,
    image: image,
    date: '2/23/24',
    title:
      'Revisit and Donation of Books to TOCMO Integrated School & Human Rights Awareness to Parents',
    texts: [text3],
  },
];
export default eventsDetailsWP3;
